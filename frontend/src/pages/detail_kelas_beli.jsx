...
export default function DetailKelasUser() {
  ...
  const newComment = {
    ...
    dibuat: res.data.dibuat || new Date().toISOString(), 
      ...
      const errorMessage = err.response?.data?.message || 'Gagal mengirim komentar.';
    ...
    <main className="min-h-screen bg-[#0a0a57] py-10 px-4 text-white">
      ...
      <div className="max-w-5xl mx-auto bg-white text-[#0a0a57] p-6 rounded-xl shadow-lg">
        ...
        <div className="grid md:grid-cols-2 gap-6 items-start mb-8">
          <img src={`http://localhost:5000/uploads/${kelas.image}`} alt="gambar kelas" className="w-full rounded-xl object-cover shadow-md"/>
          <div className="space-y-4">
            <div><h3 className="text-lg font-bold mb-1">Deskripsi</h3><p className="text-sm text-gray-700">{kelas.deskripsi}</p></div>
            <div>
              <h3 className="text-lg font-bold mb-1 flex items-center gap-2"><GiPlagueDoctorProfile /> Profil Instruktur</h3>
              <div className="flex items-center gap-4">
                {kelas.foto_pengajar_url && <img src={kelas.foto_pengajar_url} alt="Foto Pengajar" className="w-20 h-20 rounded-full object-cover border shadow"/>}
                <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {kelas.tools.map((tool) => (
                <div key={tool.id} className="flex gap-4 bg-[#eef2ff] p-4 rounded-lg shadow">
                  {tool.image ? (<img src={`http://localhost:5000/uploads/${tool.image}`} alt={tool.judul} className="w-16 h-16 object-cover rounded"/>) : (<div className="w-16 h-16 bg-gray-300 rounded flex items-center justify-center text-xs text-gray-500">No Image</div>)}
                  <div>
          ...
          {kelas.sesi?.length > 0 ? (
            kelas.sesi.map((sesi, index) => (
              <div key={sesi.id} className="bg-[#eef2ff] rounded-xl mb-4 shadow-md">
                ...
                {openSesi[sesi.id] && (<div className="px-4 pb-4 text-sm text-gray-700">{sesi.topik}</div>)}
              </div>
            ))
        ...
        <div className="mt-10">
          ...
          <div className="bg-blue-700 p-4 rounded-xl space-y-4">
            <div className="flex overflow-hidden rounded-md bg-white w-full">
              <textarea className="flex-1 p-2 text-sm focus:outline-none" placeholder="Add a comment..." value={komentarInput} onChange={(e) => setKomentarInput(e.target.value)} />
              <button onClick={handlePostKomentar} className="bg-gradient-to-r from-blue-600 to-blue-800 text-white px-5 font-semibold cursor-pointer">Post</button>
            </div>
            <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
              {!listKomentar?.length ? (
                ...
                listKomentar.map((item) => (
                  <article key={item.id} className="flex items-start gap-3 bg-white text-[#0a0a57] p-3 rounded shadow">
                    <img src={`http://localhost:5000/uploads/${item.foto || 'default-avatar.png'}`} alt="avatar" className="w-10 h-10 object-cover rounded-full border" onError={(e) => { e.target.onerror = null; e.target.src = '/default-avatar.png'; }} />
                    <div>
        ...
        <div className="text-center mt-10">
          <button onClick={() => navigate('/daftar_kelas')} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg cursor-pointer transition">Kembali</button>
        </div>
      </div>
    </main>
  );
